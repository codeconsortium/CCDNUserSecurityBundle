...
ccdn_user_security:
	...
	login_shield:
		enable_shield: true
		block_for_minutes: 10 # time in minutes we will prevent login for.
		limit_failed_login_attempts:
			before_recover_account: 25 # 25 attempts before we do not show login form.
			before_return_http_500: 50 # 50 attempts before we return an HTTP 500 error.
		primary_login_route:
			name: fos_user_security
			params:	~
